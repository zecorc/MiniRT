...
struct Vec3 {
  ...
  Vec3 operator / (double d) const { return Vec3(x/d, y/d, z/d); }
  Vec3 normalize() const {                    //normalizzaizone 
    double mg = sqrt(x*x + y*y + z*z);
    return Vec3(x/mg,y/mg,z/mg);
  }
};
inline double dot(const Vec3& a, const Vec3& b) {
  return (a.x*b.x + a.y*b.y + a.z*b.z);        //prodotto vettoriale tra 2 vettori 
}
...
int main() {
  ...
  const int H = 1500;                                     //altezza schermo
  const int W = 1500;                                     //larghezza schermo

  const Vec3 white(255, 255, 255);                        //rgb colore bianco
  const Vec3 black(0, 0, 0);                              //rgb colore nero
  const Vec3 red(255, 0, 0);                              //rgb colore rosso
